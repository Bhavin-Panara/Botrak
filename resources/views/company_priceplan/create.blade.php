...
@section('title', 'Add Organization Price Plans')
...
<div class="app-content-header">
    ...
    <div class="container-fluid">
        <!--begin::Row-->
        <div class="row">
            <div class="col-sm-6"><h3 class="mb-0">Add Organization Price Plans</h3></div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-end">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('company_priceplan.index') }}">Organization Price Plans List</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Add Organization Price Plans</li>
                </ol>
            </div>
        </div>
        <!--end::Row-->
        ...
        <div class="row g-4">
            ...
            <div class="col-md-12">
                ...
                <div class="card card-primary card-outline mb-4">
                    <!--begin::Form-->
                    <form action="{{ route('company_priceplan.store') }}" method="post">
                        @csrf
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="company_id" class="form-control">
                                            ...
                                            @foreach($users as $user)
                                                <option value="{{ $user->id }}" {{ old('company_id') == $user->id ? 'selected' : '' }}>{{ $user->name }} (Total Asset: {{ $assetCounts[$user->id] ?? 0 }})</option>
                                            @endforeach
                                        </select>
                                        @error('company_id')
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select id="price_plan_id" name="price_plan_id" class="form-control">
                                            ...
                                            @foreach($plans as $plan)
                                                <option value="{{ $plan->id }}" {{ old('price_plan_id') == $plan->id ? 'selected' : '' }}>
                                                    {{ $plan->name }}
                            ...
                            <div class="d-flex align-items-center">
                                <div class="col-md-6 pe-2">
                                    <div class="mb-3">
                                        ...
                                        <input type="date" name="start_date" class="form-control" value="{{ old('start_date') }}" min="{{ \Carbon\Carbon::today()->toDateString() }}">
                                        @error('start_date')
                                            <div class="form-text text-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
...
<script>
    $(document).ready(function () {
        ...
        if (initialPlanId) {
            PricePlan(initialPlanId);
        }

        $('#price_plan_id').on('change', function () {
            const selectedPlanId = $(this).val();
            PricePlan(selectedPlanId);
        });

        function PricePlan(planId) {
            if (planId) {
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.data) {
                            ...
                            var html = `<div class="card card-primary card-outline border-0 bg-light mb-3">
                            ...
                            if (plan.is_unlimited === "No") {
                                if (plan.tiers && plan.tiers.length > 0) {
                                    html += `<div class="card card-info card-outline border-0 bg-light mb-3">
                                        <div class="card-header">
